  ...
  return (
    <div ref={revealRef}>
      ...
      <section className="py-16 sm:py-24 lg:py-32 bg-background">
        <div className="container mx-auto px-4 sm:px-5 lg:px-10 max-w-3xl text-center">
          <p className="reveal-element text-label text-primary mb-4 tracking-widest">SOURCE STEWARDSHIP</p>
          <h2 className="reveal-element font-primary text-2xl sm:text-4xl font-semibold text-foreground mb-6">Shaping a Sustainable World.</h2>
          <p className="reveal-element font-secondary text-base sm:text-lg text-muted-foreground leading-relaxed">
        ...
        <div className="container mx-auto px-4 sm:px-5 lg:px-10">
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8">
            {[
              ...
              <div key={item.title} className="reveal-element bg-background p-8 sm:p-10 rounded-lg shadow-[var(--shadow-card)] text-center">
                <div className="font-primary text-3xl sm:text-4xl font-bold text-primary mb-2">{item.metric}</div>
                <p className="font-secondary text-xs text-muted-foreground tracking-wider uppercase mb-4 sm:mb-6">{item.metricLabel}</p>
      ...
      <section className="py-16 sm:py-24 lg:py-32 bg-background">
        <div className="container mx-auto px-4 sm:px-5 lg:px-10">
          ...
          <div className="flex gap-4 sm:gap-6 overflow-x-auto pb-4 snap-x snap-mandatory hide-scrollbar">
            {timeline.map((item) => (
              <div key={item.year} className="min-w-[240px] sm:min-w-[280px] shrink-0 snap-start bg-background border border-border rounded-lg p-6 sm:p-8 hover:shadow-[var(--shadow-card)] transition-shadow">
                <div className="font-primary text-4xl sm:text-5xl font-bold text-primary/30 mb-4">{item.year}</div>
                <h4 className="font-secondary text-base sm:text-lg font-semibold text-foreground mb-2">{item.event}</h4>
                <p className="font-secondary text-sm text-muted-foreground leading-relaxed">{item.desc}</p>
              </div>
            ))}
          </div>
      ...
      {/* CTA */}
      <section className="py-16 sm:py-20 bg-fudo-charcoal text-center">
        <div className="container mx-auto px-4 sm:px-5 lg:px-10">
          <h2 className="reveal-element font-primary text-2xl sm:text-3xl font-semibold text-primary-foreground mb-4">
            Join Fudo's journey toward a greener Gujarat.
          </h2>
          <Link
            to="/contact"
            className="reveal-element inline-flex items-center gap-2 mt-4 px-6 sm:px-8 py-3 sm:py-3.5 border border-primary-foreground/60 text-primary-foreground font-secondary text-sm font-medium tracking-widest uppercase rounded-pill transition-all duration-300 hover:bg-primary-foreground hover:text-fudo-black"
          >
            Partner With Us
          </Link>
        </div>
